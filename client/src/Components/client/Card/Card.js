...
import { usePostFavMutation } from '@/Store/User/favApi'
...
function Card(props) {
  ...
  useEffect(()=>{
    ...
    setInput({...input,cookie:cookie})
    console.log("card Cookies :",cookie)
  },[])
  const [postFav,result]=usePostFavMutation()
  ...
  return (
    <div className='flex justify-center flex-wrap w-full m-2 shadow-sm' >
      ...
      <div className='mx-3'>{locale=="ar"?props?.data?.ar_description:props?.data?.description.substr(0, 20)+"..."}</div>
      <div className=' flex items-center justify-between  text-white w-full my-4'>
        <button className='bg-green-600 rounded-md p-1 w-1/2 mx-3'>Contact</button>
        <button onClick={()=>{    postFav({cookie:input.cookie,patch:{postid:props?.data?.id}})}} className='bg-emerald-600 rounded-md p-1 w-1/2 mx-3 flex justify-center'>Add to Favoite</button>
